...
class ElementTransform
{
    ...
    {
        if (isset($node->props['item_animation'])) {
            return;
        }
        ...
        $svgAnimation = !empty($node->props['image_inline_svg']) && !empty($node->props['image_animate_svg'])
            ? 'uk-active'
            : false;

        if ($sectionAnimation) {
            $node->attrs['uk-scrollspy-class'] = !empty($node->props['animation'])
                ? ['uk-animation-{!animation: none|parallax}', $svgAnimation]
                : ($svgAnimation
                    ? "uk-animation-{$sectionAnimation} {$svgAnimation}"
                    : true);
        } elseif ($svgAnimation) {
            $node->attrs['uk-scrollspy'] = ['cls: {0};' => $svgAnimation];
        }

        if ($svgAnimation) {
            $node->attrs['class'][] = 'uk-animation-active';
            $node->props['image_inline_svg'] = ['stroke-animation: true;'];
        }
    ...
    public function customCSS($node, array $params)
    {
        ...
        $this->theme->set('css', $this->theme->get('css', '') . $css);
    }
